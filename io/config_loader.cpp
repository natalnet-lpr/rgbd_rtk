...
void ConfigLoader::loadParams(string filename){
  ...
  try{//Loading Params
    ...
    fs["aruco_minimum_distance"] >> aruco_minimum_distance_;
    fs["aruco_marker_size"] >> aruco_marker_size_;
    //looking if any of the params was not loaded
    if(camera_calibration_file_.empty() || index_file_.empty() ||aruco_dic_.empty() || aruco_minimum_distance_ == 0 || aruco_marker_size_ == 0) throw 1;
    ...
    cout<<"aruco_minimum_distance: "<<aruco_minimum_distance_<<endl<<"aruco_marker_size: "<<aruco_marker_size_<<endl;
  }
