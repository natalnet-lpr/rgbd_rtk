...
GeometricMotionSegmenter::GeometricMotionSegmenter(const vector<cv::Point2f> &curr_pts,
...
bool GeometricMotionSegmenter::isDynamicPoint(const PointT &pt_from, const PointT &pt_to, const float threshold)
{
    ...
    const double dist = cv::norm(cv::Mat(cv_pt_from), cv::Mat(cv_pt_to));
...
void GeometricMotionSegmenter::calculateDynamicPoints(const pcl::PointCloud<PointT> &sparse_cloud_src,
                                                      const pcl::PointCloud<PointT> &sparse_cloud_tgt,
                                                      const Eigen::Affine3f &clouds_relative_pose,
                                                      const std::vector<int> &mapper_2d_3d,
                                                      const float threshold)
{
    // apply the relative transformation in the cloud
    pcl::PointCloud<PointT> transformed_cloud_src;
    pcl::transformPointCloud(sparse_cloud_src, transformed_cloud_src, clouds_relative_pose);

    double z = std::numeric_limits<double>::max();
    int saved_idx = -1;

    // loop over all cloud points and check if it is a dynamic point
    for (int i = 0; i < std::min(sparse_cloud_tgt.size(), sparse_cloud_tgt.size()); i++)
    {
        const auto pt_from = (transformed_cloud_src)[i];
        const auto pt_to = (sparse_cloud_tgt)[i];

        // get the index based on tracker

        int idx = (mapper_2d_3d)[i];
        /*print = false;
        if( z > pt_to.z)
        {
            z = pt_to.z;
...
void GeometricMotionSegmenter::calculateDynamicPoints()
{
    ...
    mappers_2d_3d_.push_back(mapper_2d_3d_);
    ...
    size_t cloud_size = curr_sparse_cloud_->size();
    ...
    for (int i = sparse_clouds_pairs_.size() - 1; i >= 0; i--)
    {
        ...
        if (src_cloud.size() > cloud_size || tgt_cloud.size() > cloud_size)
        {
            src_cloud.resize(cloud_size);
            tgt_cloud.resize(cloud_size);
        }
    ...
    // loop
    auto static_pts_idxs = getStaticPointsIndex();

    for (const auto &idx : static_pts_idxs)
    {
        const auto &curr_pt = (curr_pts_)[idx];
        const auto &prev_pt = (prev_pts_)[idx];
        curr_static_pts_->push_back(curr_pt);
        prev_static_pts_->push_back(prev_pt);
    }

    auto dyna_pts_idxs = getDynamicPointsIndex();
    for (const auto &idx : dyna_pts_idxs)
    {
        const auto &curr_pt = (curr_pts_)[idx];
        dyna_pts_->push_back(curr_pt);
    }
}
