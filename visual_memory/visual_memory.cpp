...
void VisualMemory::add(cv::Mat trainDescriptors, int frame_idx, Eigen::Matrix4f pose)
{
    //trainDescriptorsSet_.push_back(trainDescriptors);
    matcher_->add(trainDescriptors);
...
bool sort_assistant(std::pair<int, int> a, std::pair<int, int> b)
{
...
std::vector<int> VisualMemory::searchDescriptor(cv::Mat queryDescriptor, int n, cv::Ptr<cv::DescriptorMatcher> matcher)
{
    ...
    for (int i = 0; i < n; i++)
    {
        std::cout << "índice da imagem na sequência " << cont[i].second << " quantidade de matchings " << cont[i].first << std::endl;
        output.push_back(cont[i].second);
    }
    return output;
}
std::vector<int> VisualMemory::searchImage(cv::Mat img)
{
    ...
    return searchDescriptor(descriptors, 10);
}